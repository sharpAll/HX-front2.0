...
export default {
  ...
  methods: {
    ...
    onCanvasClick(e) {
      ...
      if (this.clicked_camera(raycaster, e)) {
        alert("clicked_camera - returned a clicked -- ");
        return;
      }
    },
    clicked_camera(raycaster, e) {
      //与camera求交
      var cameraIntersects = raycaster.intersectObjects(
        this.camera_label.children,
        true
      );
      if (!cameraIntersects.length) {
        return false;
      }
      return true;
    },
    loadCamera() {
      ...
      z = parseInt(-4);
      type = "camera_2";
      this.createCamera(name, x, y, z, this.camTexture[type]);
      this.scene.add(this.camera_label);
    },
    createCamera(name, posX, posY, posZ, texture) {
      ...
      sprite.name = name; //以序号给camera命名
      this.camera_label.add(sprite);
      this.camera_label_arr.push(sprite);
    },
    onMouseMove(e) {
      this.mouse.x =
        ((e.clientX - this.container.getBoundingClientRect().left) /
      ...
      var raycaster = new THREE.Raycaster();
      raycaster.setFromCamera(this.mouse, this.camera);
      var ifMouseOnCam = this.mouseOnCam(raycaster);
      if (ifMouseOnCam) {
    ...
    mouseOnCam(raycaster) {
      var cameraIntersects;
      cameraIntersects = raycaster.intersectObjects(
        this.camera_label.children,
        true
      );
      if (cameraIntersects.length) {
